...
class AddController extends AbstractController
{
    ...
    public function addMovie(Request $request, EntityManagerInterface $em, TmdbService $tmdb, TranslatorInterface $translator) : Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $name = (new Slugify)->slugify($post['title']) . '.jpg';
            $tmdb->downloadFile($posters['big'], $this->getParameter('postersDir'), $name);
            $tmdb->downloadFile($posters['small'], $this->getParameter('postersThumbs'), $name);
